...
<body>
    ...
    <p class="note"><b>Note:</b>API Key REQUIRED through <a href="https://serpapi.com/" target="_blank">
        SERPAPI - Google Custom Search API</a>. <br>
        Please save it in a secure password manager - the information is <b>not</b> stored on the server.</p>
    ...
    <form method="post" action="">
        ...
        <label for="sites">Optional: Specific websites. Rec: Use for frequent offenders. (Separate entries with comma + space)</label>
        <input type="text" id="sites" name="sites" value="<?php echo htmlspecialchars($siteInput, ENT_QUOTES, 'UTF-8'); ?>" placeholder="leakienudies.com, ibridroomies.com, etc.">

        <label for="exact">Optional: Exact matches. Rec: Use for your alias, stage name, or impersonator's name (Separate with Comma + Space)</label>
        <input type="text" id="exact" name="exact" value="<?php echo htmlspecialchars($exactInput, ENT_QUOTES, 'UTF-8'); ?>" placeholder="Stage Name, Videogame Handle, etc.">

        <label for="pages">How many result pages? Each page removes up 1-2 searches through serpapi.</label>
        <input type="number" id="pages" name="pages" min="1" max="10" value="<?php echo (int)$pages; ?>">

        <label for="safe">SafeSearch? (Recommended: OFF to detect deepfakes and leaks.) </label>
